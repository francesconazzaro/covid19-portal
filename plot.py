...
def plot_fit(data, fig, t_0, t_d, r2, start=None, stop=None, label=None, shift=5, **kwargs):
    ...
    x_base = np.arange(x[0] - np.timedelta64(shift, 'D'), x[-1] + np.timedelta64(shift, 'D'),
                       np.timedelta64(1, 'D'))
    trace = go.Line(x=x_base, y=stats.exp2(x_base, t_0, t_d), name=label, visible="legendonly", **kwargs)
    fig.add_trace(trace, 1, 1)
    return fig


# def prediction(x, t_0, t_d, r2, label, ax, days_after=1, func=exp2, **plot_kwargs):
#     ax.plot(x, exp2(x, t_0, t_d), linestyle=':', **plot_kwargs)
#     x_predict = x[-1] + np.timedelta64(days_after, 'D')
#     y_predict = func(x_predict, t_0, t_d)
#     if label is not None:
